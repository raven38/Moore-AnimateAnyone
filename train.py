...
class Net(nn.Module):
    def __init__(
        ...
        pose_guider: PoseGuider | PoseModulation,
...
def log_validation(
    ...
    test_cases = [
        (
            ...
            "./configs/inference/env_videos/anyone-video-1_env.mp4",
            "./configs/inference/pose_videos/anyone-video-1_kps.mp4",
            "./configs/inference/depth_videos/anyone-video-1_depth.mp4",
            "./configs/inference/seg_videos/anyone-video-1_seg.mp4", 
        ),
        (
            "./configs/inference/ref_images/anyone-2.png",
            "./configs/inference/env_videos/anyone-video-2_env.mp4",
            "./configs/inference/pose_videos/anyone-video-2_kps.mp4",
            "./configs/inference/depth_videos/anyone-video-2_depth.mp4",
            "./configs/inference/seg_videos/anyone-video-2_seg.mp4",
        ),
    ]

    results = []
    for test_case in test_cases:
        ref_image_path, env_video_path, pose_video_path, depth_video_path, seg_video_path = test_case
        ...
        env_list = []
        env_tensor_list = []
        env_images = read_frames(env_video_path)
        env_transform = transforms.Compose(
            [transforms.Resize((height, width)), transforms.ToTensor()]
        )
        for env_image_pil in env_images[:clip_length]:
            env_tensor_list.append(env_transform(env_image_pil))
            env_list.append(env_image_pil)
        
        env_tensor = torch.stack(env_tensor_list, dim=0)  # (f, c, h, w)
        ...
        depth_list = []
        depth_tensor_list = []
        depth_images = read_frames(depth_video_path)
        depth_transform = transforms.Compose(
            [transforms.Resize((height, width)), transforms.ToTensor()]
        )
        for depth_image_pil in depth_images[:clip_length]:
            depth_tensor_list.append(depth_transform(depth_image_pil))
            depth_list.append(depth_image_pil)

        depth_tensor = torch.stack(depth_tensor_list, dim=0)  # (f, c, h, w)
        depth_tensor = depth_tensor.transpose(0, 1)
        ...
        for seg_image_pil in seg_images[:clip_length]:
            seg_tensor_list.append(seg_transform(seg_image_pil))
            seg_list.append(seg_image_pil)
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                pixel_values_depth = batch["pixel_values_depth"] # (bs, f, c, H, W)
                pixel_values_depth = pixel_values_depth.transpose(
                    1, 2
                )  # (bs, c, f, H, W)
